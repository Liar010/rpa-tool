...
public partial class ScrollUntilImageDialog : Window
{
    public ScrollUntilImageAction Action { get; private set; }
    private string? _currentScriptPath;
    private readonly ScriptEngine? _scriptEngine;

    public ScrollUntilImageDialog(string? currentScriptPath = null, ScriptEngine? scriptEngine = null)
    {
        InitializeComponent();
        Action = new ScrollUntilImageAction();
    ...
    public ScrollUntilImageDialog(ScrollUntilImageAction existingAction, string? currentScriptPath = null, ScriptEngine? scriptEngine = null)
        : this(currentScriptPath, scriptEngine)
    {
        // 既存のアクションの値を設定（編集モード）
        rbDirectionDown.IsChecked = existingAction.Direction == ScrollDirection.Down;
        rbDirectionUp.IsChecked = existingAction.Direction == ScrollDirection.Up;
        rbDirectionRight.IsChecked = existingAction.Direction == ScrollDirection.Right;
        rbDirectionLeft.IsChecked = existingAction.Direction == ScrollDirection.Left;

        txtTemplatePath.Text = existingAction.TemplateImagePath ?? "";
        txtScrollAmount.Text = existingAction.ScrollAmount.ToString();
        txtMaxScrollCount.Text = existingAction.MaxScrollCount.ToString();
        txtWaitAfterScroll.Text = existingAction.WaitAfterScrollMs.ToString();
        txtMatchThreshold.Text = existingAction.MatchThreshold.ToString("F2");
        chkMultiScale.IsChecked = existingAction.UseMultiScale;
        chkClickWhenFound.IsChecked = existingAction.ClickWhenFound;
    ...
    private void BtnOK_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (rbDirectionUp.IsChecked == true) direction = ScrollDirection.Up;
        else if (rbDirectionRight.IsChecked == true) direction = ScrollDirection.Right;
        else if (rbDirectionLeft.IsChecked == true) direction = ScrollDirection.Left;
        ...
        // スクロール量
        if (!int.TryParse(txtScrollAmount.Text, out int scrollAmount) || scrollAmount <= 0)
        {
            MessageBox.Show("スクロール量は正の値で指定してください", "エラー", MessageBoxButton.OK, MessageBoxImage.Error);
            return;
        }

        // 最大スクロール回数
        if (!int.TryParse(txtMaxScrollCount.Text, out int maxScrollCount) || maxScrollCount <= 0)
        {
            MessageBox.Show("最大スクロール回数は正の値で指定してください", "エラー", MessageBoxButton.OK, MessageBoxImage.Error);
            return;
        }

        // スクロール後待機時間
        if (!int.TryParse(txtWaitAfterScroll.Text, out int waitAfterScroll) || waitAfterScroll < 0)
        {
            MessageBox.Show("スクロール後待機時間が無効です", "エラー", MessageBoxButton.OK, MessageBoxImage.Error);
            return;
        }
        ...
        Action = new ScrollUntilImageAction
        {
            ...
            WaitAfterScrollMs = waitAfterScroll,
            MatchThreshold = threshold,
            UseMultiScale = chkMultiScale.IsChecked == true,
            ClickWhenFound = chkClickWhenFound.IsChecked == true,
